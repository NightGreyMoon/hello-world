...
export function getBetweenDates(count?: number, year?: number, month?: number, day?: day) {
  if (year === undefined || month === undefined || day === undefined) {
    return request<DateOptions[]>({
      url: `/Date/GetBetween?count=${count}`,
      method: 'get'
    });
  }

  return request<DateOptions[]>({
    url: `/Date/GetBetween?count=${count}&date=${year}-${month}-${day}`,
    method: 'get'
  });
}
